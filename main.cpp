...
#pragma region DirectX初期化処理
	...
	ID3D12CommandAllocator* commandAllocastor = nullptr;
	...
	//DXGIファクトリーの生成
	result = CreateDXGIFactory(IID_PPV_ARGS(&dxgiFactory)); assert(SUCCEEDED(result));
	// アダプターの列挙用
	...
	result = device->CreateCommandAllocator(
		...
		IID_PPV_ARGS(&commandAllocastor));
	assert(SUCCEEDED(result));
	// コマンドリストを生成 
	result = device->CreateCommandList(0,
		D3D12_COMMAND_LIST_TYPE_DIRECT,
		commandAllocastor, nullptr,
...
#pragma region 描画処理初期化
	...
	blenddesc.DestBlend = D3D12_BLEND_INV_SRC_ALPHA;//1.0f-のソースのアルファ値
	//頂点レイアウトの設定
	...
	descriptorRange.NumDescriptors = 1;		//一度の描画に使うテクスチャが一枚なので
	...
	cbHeapProp.Type = D3D12_HEAP_TYPE_UPLOAD;	//CPUへの転送用
	//リソース設定
...
#pragma endregion
	...
	#pragma region 描画コマンド
		...
		D3D12_GPU_DESCRIPTOR_HANDLE srvGpuHandle = srvHeap->GetCPUDescriptorHandleForHeapStart();
...
#pragma endregion 
	...
	// キューをクリア 
	result = commandAllocastor->Reset();
	assert(SUCCEEDED(result));
	// 再びコマンドリストを貯める準備 
	result = commandList->Reset(commandAllocastor, nullptr);
	assert(SUCCEEDED(result));
#pragma endregion
